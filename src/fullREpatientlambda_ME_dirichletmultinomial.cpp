// several lambdas - one for each patient - in a DM model with correlated random effects
...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER_MATRIX(log_lambda); // log of the parameter for overdispersion in Dirichlet-Multinomial model (n values, one for each patient)
  int d_min1 = d - 1;

  // for covariance matrix for random effects
  using namespace density;

  matrix<Type> log_lambda_vec(num_individuals, 1);
  ...
  log_lambda_vec = z * log_lambda; // get a vector of lambdas of length p*n based on the patient-specific lambdas

  for(int i=0;i<num_individuals;i++){ // likelihood for the random effects (multivariate normal)
    nll += VECSCALE(nll_mvn, exp(logs_sd_RE))(u_large.row(i));
  }
